...
export function BottomCtaC3() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('[bottom-cta-c3] Submission failed:', res.statusText)
      }
    } catch (err) {
      console.error('[bottom-cta-c3] Network error:', err)
    } finally {
      // Push conversion event to GTM dataLayer before redirect
      if (typeof window !== 'undefined') {
        ;(window as any).dataLayer = (window as any).dataLayer || []
        ;(window as any).dataLayer.push({
          event: 'form_submission',
          form_source: 'lp3-bottom',
  ...
  return (
    <section className="py-20 md:py-24" style={{ background: '#111111' }}>
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          Comparing interior companies? Talk to DeX first.
        </h2>
        ...
        >
          Bring your brief. We&apos;ll show you our process, materials, and guarantee — in person at our experience centre or over a call.
        </p>
